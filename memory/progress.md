...
## Session log
- [x] 2026-07-19 — **Change-password hardening** (ad-hoc, owner-approved). Auditoria do `POST /users/me/change-password` revelou 2 defeitos: (1) exigia o refresh token no **body**, mas o refresh fica em cookie **HttpOnly** — o frontend não consegue lê-lo, então o endpoint era inutilizável pela SPA; (2) revogava só a sessão passada e deixava as outras vivas (inseguro numa troca de senha).
  - Decisão de design (owner "faça tudo isso"): como a troca passa a **revogar todas as sessões**, o cliente não precisa enviar refresh token nenhum → removido o campo `refreshToken` do `ChangePasswordRequest` (agora só `currentPassword` + `newPassword`).
  - Produção: `UserService.changeMyPassword` valida senha atual → salva nova (BCrypt) → `refreshTokenService.deleteAllSessions(email)`. Novo `RefreshTokenService.deleteAllSessions(userEmail)` delega ao `RefreshTokenRepository.deleteByUserEmail` (já existia, com `@Indexed` no `userEmail`). `UserControllerDocs` atualizado (revoga todas as sessões; 422 só p/ senha incorreta). `UserService`/`UserServiceTest` perderam o import de `RefreshToken`.
  - TDD: testes reescritos antes (vermelho por compilação, depois verde). `UserServiceTest$ChangeMyPassword` reduziu de 4 p/ 2 casos (removidos os 2 de ownership/missing do refresh token; happy path agora verifica `deleteAllSessions`). `UserControllerTest` ajustado p/ ctor de 2 args. `RefreshTokenServiceTest` +1 caso (`deleteAllSessions` delega ao repo).
  - Doc frontend: criado `docs/frontend-user-creation.md` (guia de criação de usuário ADMIN-only + nova seção de troca de senha explicando a revogação total e o fluxo de logout+redirect).
  - Suíte: **245/245 verde** (era 246 antes; -1 net = 2 testes de refresh token removidos + 1 novo no RefreshTokenServiceTest).
  - Commits (owner aprovou a divisão): (1) `fix(user): revoke all sessions on password change and drop client refresh token` — DTO/service/refreshtoken/docs + 3 testes; (2) `docs(frontend): document password-change flow and session revocation` — `docs/frontend-user-creation.md`.
- [~] 2026-07-15 — **Prod-readiness plan** (`.claude/prod-readiness-plan.md`) started. Owner decisions: subdomínios same-site (`app.` / `api.`), `/register` → ADMIN, plano escrito antes de codar. 4 fases: (1) profile prod config, (2) hardening auth, (3) Docker/rede, (4) operação.
  ...
  - [~] **Fase 4 — operação (parcial; owner adiou o resto em 2026-07-15).**
    - [x] **Seed do admin por env (shipped `446b7e2`).** Resolve o bootstrap chicken-egg: como `/register` exige ADMIN, um banco novo não teria como criar o primeiro usuário. Novo `AdminSeeder` (`CommandLineRunner`, `@Order(2)`, roda após `DataInitializer` `@Order(1)`) cria o admin a partir de `worklog.admin.{email,password,name}` (env `ADMIN_EMAIL`/`ADMIN_PASSWORD`/`ADMIN_NAME`) se ainda não existir; senha via `PasswordEncoder`, role ADMIN. Idempotente; desligado quando email/senha em branco (dev/test). `AdminSeedProperties` (record `@ConfigurationProperties("worklog.admin")`) habilitado no `SecurityConfig`. Base yaml ganhou `worklog.admin.*` com defaults vazios. `.env.prod.example` atualizado. Sem ciclo de beans (contexto sobe ok — `AdminSeeder` não participa do grafo do `CustomUserDetailsService`).
      - TDD: `AdminSeederTest` (5 casos: cria quando ausente/config; ignora se já existe; no-op se email ou senha vazios; lança `IllegalStateException` se role ADMIN sumir). Suíte 241 → 246.
      - **Smoke test real na stack prod:** admin semeado no boot (`Usuário ADMIN inicial criado`), login → 204 + cookie `Secure;HttpOnly;SameSite=Strict` com `ROLE_ADMIN` no JWT, senha errada → 403, e 2º boot idempotente (`seed ignorado: já existe`). Stack derrubada, env descartável removido, dev intacto.
      - Shipped as `446b7e2` `feat(auth): seed initial ADMIN user from env on startup`.
      - Manejo seguro de credencial (owner: VPS única, nível 1): `.env.prod` `chmod 600`; senha do admin é efêmera (só usada no 1º boot) → trocar via `POST /users/me/change-password` no 1º login e remover `ADMIN_PASSWORD` do `.env.prod`. Não foi adicionado suporte a `*_FILE`/Docker secrets (desnecessário p/ VPS única).
    - [x] **`DEPLOYMENT.md` (raiz, GITIGNORED — local, não versionado por decisão do owner).** Runbook completo pt-BR: endurecer VPS (usuário não-root, SSH por chave, ufw, unattended-upgrades, fail2ban, swap), Cloudflare (DNS proxied, SSL Full strict, Origin Certificate, ufw restrito às faixas CF), deploy (clone, `.env.prod` chmod 600, `up --build`), nginx (real-IP CF + proxy p/ 127.0.0.1:8080 + TLS origin), verificação externa, pós-deploy (troca senha admin + remove `ADMIN_PASSWORD`), operação (update/backup/logs) e checklist. Rate limit fica na Cloudflare (decisão do owner). Adicionado `DEPLOYMENT.md` ao `.gitignore`.
    - [ ] Em aberto (adiados): rate limit (→ Cloudflare, resolvido); healthcheck via Actuator; frontend/CORS (repo separado).
