...
- [~] 2026-07-15 — **Prod-readiness plan** (`.claude/prod-readiness-plan.md`) started. Owner decisions: subdomínios same-site (`app.` / `api.`), `/register` → ADMIN, plano escrito antes de codar. 4 fases: (1) profile prod config, (2) hardening auth, (3) Docker/rede, (4) operação.
  ...
  - [ ] **Fase 4 — operação (adiada por decisão do owner em 2026-07-15).** Backend considerado pronto para rodar; Fase 4 fica para depois. Itens em aberto quando/se retomar: (1) rate limit no `/login` — bucket4j no backend OU `limit_req` no nginx; (2) healthcheck via Actuator (`/actuator/health`) p/ o `healthcheck` do compose e do proxy; (3) reverse proxy nginx/Caddy + TLS + repasse de cookies — provavelmente vive no repo de infra, não neste. Owner optou por não escrever código nem gerar exemplo de proxy agora.
